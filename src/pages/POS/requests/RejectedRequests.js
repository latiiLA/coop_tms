...
export default function RejectedRequests() {
  ...
  const fetchRows = async () => {
    ...
    try {
      const response = await axios.get(`${apiUrl}/request/getRejectedRequest`, {
        headers: {
    ...
    >
      ...
      >
        <Tabs value={value} onChange={handleChange} aria-label="Rejection Tabs">
          <Tab label="All Rejected Requests" />
          <Tab label="Rejected Merchant POS" />
          <Tab label="Rejected Branch POS" />
        </Tabs>
        <TextField
          label="Search"
          value={searchText}
