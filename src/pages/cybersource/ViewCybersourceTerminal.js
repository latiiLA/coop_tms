...
export default function ViewCybersourceTerminal() {
  ...
  const fetchRows = async () => {
    ...
    try {
      const response = await axios.get(`${apiUrl}/cybersource/getCybersourceTerminal`, {
        headers: {
    ...
    <Box>
      ...
      >
        <Tabs value={value} onChange={handleChange} aria-label="Cybersource Tabs">
          <Tab label="All Cybersources" />
          <Tab label="Ecommerce Cybersources" />
          <Tab label="AFT Cybersources" />
        </Tabs>
        <TextField
          label="Search"
          value={searchText}
